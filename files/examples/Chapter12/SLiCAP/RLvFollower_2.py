...
htmlPage('Root locus')
fig_PZ = plotPZ('RL_' + fileName, fileName, plots, xmin=-2, xmax=0, ymin=-1, ymax=1, xscale='M', yscale='M', show=True)
fig2html(fig_PZ, 600)
i1.setGainType('gain')
i1.setDataType('pz')
pzGain = i1.execute()
...
A = i1.execute();
plots = [A, L, S, G];
BodeMag  = plotSweep('dBmag_' + fileName, fileName, plots, 1e4, 1e8, 200, funcType='dBmag',  show=True)
BodePhas = plotSweep('Phase_' + fileName, fileName, plots, 1e4, 1e8, 200, funcType='phase', show=True)
